...
def calculate_accuracies(df, dataset):
    print(df)
    print(dataset.ans_to_ix)
    df["prediction class"] = df["prediction"].apply(lambda x: dataset.ans_to_ix[str(x)])
    df["target class"] = df["target"].apply(lambda x: dataset.ans_to_ix[str(x)])
