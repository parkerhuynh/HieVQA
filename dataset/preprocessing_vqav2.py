...
def question_token_load(question_list, args):
...
def get_top_answers(examples, args):
    ...
    answer_list = [ans[1] for ans in cw[:max_freq]]


    answer_super_types = json.load(open(args.data_config["super_type"], 'r'))
    ids = {}
    for ex in examples:
        answer_str = prep_ans(ex["multiple_choice_answer"])

        if answer_str in answer_list:
            answer_str = prep_ans(ex["multiple_choice_answer"])
            answer_type = answer_super_types[answer_str]
            if answer_type not in ids:
                ids[answer_type] = 0
            if answer_type not in ans2tok:
                ans2tok[answer_type] = {}
                tok2ans[answer_type] = {}

            if answer_str not in ans2tok[answer_type]:
                ans2tok[answer_type][answer_str] = ids[answer_type]                
                tok2ans[answer_type][ids[answer_type]] = answer_str
                ids[answer_type] += 1
    return ans2tok, tok2ans, answer_list
...
def annotation_vocal_load(anns, args):
    annotation_path = args.data_config['annotation_dict']
    
    if os.path.exists(annotation_path):
        # print("3"*100)
        print(f'> Loading {args.dataset} annotation dictionary'.upper())
        ans_to_ix, ix_to_ans, answer_list = json.load(open(annotation_path, 'r'))
    else:
        print(f'> Create and save {args.dataset} annotation dictionary'.upper())
        ans_to_ix, ix_to_ans, answer_list = get_top_answers(anns,args)
        json.dump([ans_to_ix, ix_to_ans, answer_list], open(annotation_path, 'w'))
            
        # ans_to_ix, ix_to_ans = answer_dict(anns, args, annotation_path)
    return ans_to_ix, ix_to_ans, answer_list
